...
(function () {
  ...
  EV.DATA.addSkills([
    ...
    {
      id: 'c_lash', stage: S, slot: 'active', name: 'Fagositoz', icon: '🦠', tags: ['asit'],
      kind: 'engulf', cost: 18, cd: 7,
      flavor: 'Yalancı ayağını uzatıp küçük bir avı bütünüyle yutarsın; içinde sindirirken iyileşirsin. Tekrar bas: asitle kaplı avı nişana tükür. İri avları yutamaz, asitle tokatlar.',
      base: { range: 8, maxSize: 1.6, digest: 4, tick: 0.5, dmg: 0.3, heal: 0.5, spit: 2.0, splash: 1.2, splashR: 3, spitRange: 18, st: [['vuln', 1]] },
      ranks: [
        { txt: 'Uzanma 10m, sindirim %40/tık', range: 10, dmg: 0.4 },
        { txt: 'Daha iri avları da yutar', maxSize: 2.1 },
        { txt: 'Tükürük %300, Kırılganlık 2 + Zehir', spit: 3.0, st: [['vuln', 2], ['poison', 2]] },
        { txt: 'Bekleme 5sn, sindirim %50/tık', cd: 5, dmg: 0.5 },
      ],
    },
    {
      id: 'c_web', stage: S, slot: 'active', name: 'Yapışkan İplik', icon: '🕸️', tags: ['yapiskan'],
      kind: 'tether', cost: 16, cd: 6,
      flavor: 'Ava yapışkan bir iplikle bağlanırsın: yavaşlatır, özünü emip seni iyileştirir. Çok uzaklaşırsan kopar; süre dolarsa iplik patlar.',
      base: { range: 14, breakR: 18, dur: 4, tick: 0.5, dmg: 0.25, heal: 0.3, end: 1.2, st: [['slow', 1]], endSt: [['slow', 2]] },
      ranks: [
        { txt: 'Hasar %32/tık, kopma 22m', dmg: 0.32, breakR: 22 },
        { txt: 'İplik avı sana doğru çeker', pull: 2.5 },
        { txt: 'Av ölünce iplik en yakındakine sıçrar', jump: 2 },
        { txt: 'Süre 5sn, bitiş patlaması %250 + Kırılganlık', dur: 5, end: 2.5, endSt: [['slow', 2], ['vuln', 1]] },
      ],
    },
    {
      id: 'c_cilia', stage: S, slot: 'active', name: 'Kirpik Dönüşü', icon: '🌀', tags: ['fiziksel', 'zar'],
      kind: 'charge', cost: 18, cd: 5,
      flavor: 'Basılı tut: kirpiklerin hızlanır, alan büyür. Bırak: çevreni süpürüp savurursun. Tam dolumda sersemletir.',
      base: { shape: 'nova', full: 1.0, min: 0.35, dmg: 3.0, r: 6.5, knock: 14, selfSlow: 0.35, autoHold: 1.0, st: [], stFull: [['stun', 0.6]] },
      ranks: [
        { txt: 'Alan 7.3m', r: 7.3 },
        { txt: 'Hasar %380, yavaşlatır', dmg: 3.8, st: [['slow', 1]] },
        { txt: 'Dolum 0.75sn', full: 0.75 },
        { txt: 'Hasar %480, Yavaşlama 2', dmg: 4.8, st: [['slow', 2]] },
      ],
    },
    {
    ...
    {
      id: 'c_membrane', stage: S, slot: 'active', name: 'Zar Gerilimi', icon: '🛡️', tags: ['zar'],
      kind: 'parry', cost: 12, cd: 6,
      flavor: 'Darbe gelmeden hemen önce bas: zarın gerilir, gelen vuruşu tamamen emer ve saldıranı elektrikle geri teper. Başarırsan bekleme yarıya iner.',
      base: { win: 0.5, counter: 3.0, knock: 12, refund: 0.5, st: [['stun', 1]] },
      ranks: [
        { txt: 'Pencere 0.6sn', win: 0.6 },
        { txt: 'Karşı darbe %400', counter: 4 },
        { txt: 'Başarıda %20 kalkan', shield: 0.2 },
        { txt: 'Karşı darbe çevreye yayılır (4m), sersemlik 1.4sn', counterR: 4, splash: 2, st: [['stun', 1.4]] },
      ],
    },
    {
    ...
    {
      id: 'c_zap', stage: S, slot: 'active', name: 'Kamçı Motoru', icon: '💨', tags: ['elektrik'],
      kind: 'rush', cost: 18, cd: 5,
      flavor: 'Basılı tut: kamçın hızlanır, nişanınla yön verirsin; çarptıklarını ezip yana savurursun. Bırakınca hızınla orantılı elektrik darbesi.',
      base: { v0: 12, vmax: 34, accel: 40, maxT: 1.5, turn: 3.2, width: 2.2, trample: 0.6, dmg: 2.6, r: 3.5, knock: 10, armor: 0.3, autoHold: 0.7, st: [['shock', 1]] },
      ranks: [
        { txt: 'Daha seri hızlanır', accel: 55 },
        { txt: 'Darbe %330, Şok 2', dmg: 3.3, st: [['shock', 2]] },
        { txt: 'Bekleme 4sn', cd: 4 },
        { txt: 'Ezme %100, darbe %360', trample: 1.0, dmg: 3.6 },
      ],
    },
    {
