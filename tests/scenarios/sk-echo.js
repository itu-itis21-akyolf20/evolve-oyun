...
// ---- sk-echo: echo casting of every skill (incl. dash/leap/hunt must not move player) ----
const out = [];
[0, 1, 2].forEach((s) => {
  gotoStage(s);
  ...
  defs.forEach((def) => {
    [1, 3].forEach((rank) => {
      const row = { id: def.id, kind: def.kind, rank };
      try {
        resetArena();
        const tg = spawnTargets(s);
        const P = G.player;
        const start = P.group.position.clone();
        const aimYaw0 = P.aimYaw;
        G.legacy.echoes = [{ id: def.id, rank, t: 0 }];
        let maxMove = 0, motion = false;
        tick(1);
        row.echoT = +G.legacy.echoes[0].t.toFixed(2);
        tick(89, () => {
          maxMove = Math.max(maxMove, P.group.position.distanceTo(start));
          if (P.dash || P.leap || P.hunt) motion = true;
        });
        row.dmg3s = Math.round(hpDmg(tg));
        row.maxMove = +maxMove.toFixed(3);
        row.motionState = motion;
        row.buff = P.buffs.some((b) => b.id === def.id);
        row.allies = G.enemies.filter((e) => e.ally && e.alive).length;
        row.counts = EV.Skills.counts;
        row.nan = nanCheck(tg);
