...
// ---- sk-react: all reactions + fusion eligibility ----
const res = { reactions: [], fusions: [] };
gotoStage(1);
const CFGS = EV.CFG.STATUS;
const amount = (id) => (CFGS[id].type === 'timed' ? 1.5 : 1);
EV.CFG.REACTIONS.forEach((R) => {
  [false, true].forEach((lowHp) => {
    const row = { id: R.id, a: R.a, b: R.b, lowHp };
    try {
      resetArena();
      const tg = spawnTargets(1);
      const e = tg[3];
      if (lowHp) e.hp = e.maxHp * 0.2;
      const P = G.player;
      P.hp = P.stats.maxHp * 0.5;
      const hp0 = P.hp;
      const pow = P.stats.dmg;
      const others0 = tg.filter((o) => o !== e).map((o) => o.hp);
      const zones0 = EV.Skills.counts.zones;
      if (R.a === 'bleed' || R.b === 'bleed') EV.Status.apply(G, e, 'bleed', 2, pow, true);
      const eHp0 = e.hp;
      REACTS = 0;
      EV.Status.apply(G, e, R.a, amount(R.a), pow, true);
      const midHp = e.hp;
      EV.Status.apply(G, e, R.b, amount(R.b), pow, true);
      row.fired = REACTS;
      row.instantDmg = Math.round(eHp0 - (e.alive ? e.hp : 0));
      row.killed = !e.alive;
      row.stun = !!(e.st && e.st.stun);
      row.stunT = e.st && e.st.stun ? +e.st.stun.t.toFixed(2) : 0;
      row.sepsisT = e.sepsisT || 0;
      row.bleedN = e.st && e.st.bleed ? e.st.bleed.n : 0;
      row.poisonLeft = e.st && e.st.poison ? e.st.poison.n : 0;
      row.zonesSpawned = EV.Skills.counts.zones - zones0;
      row.othersDmg = Math.round(tg.filter((o) => o !== e).reduce((s, o, i) => s + (others0[i] - o.hp), 0));
      row.heal = Math.round(P.hp - hp0);
      row.combo = !!G.legacy.combos[R.id];
      tick(60);
      row.dmg2s = Math.round(eHp0 - (e.alive ? e.hp : 0));
      row.othersDmg2s = Math.round(tg.filter((o) => o !== e).reduce((s, o, i) => s + (others0[i] - o.hp), 0));
      row.nan = nanCheck(tg);
    } catch (err) { row.exc = shortErr(err); }
    res.reactions.push(row);
  });
});

// same reaction twice within cooldown -> should not fire twice
{
  resetArena(); const tg = spawnTargets(1); const e = tg[3]; REACTS = 0;
  const pow = G.player.stats.dmg;
  EV.Status.apply(G, e, 'slow', 1, pow, true); EV.Status.apply(G, e, 'shock', 1, pow, true);
  EV.Status.apply(G, e, 'shock', 1, pow, true);
  const n1 = REACTS; tick(50); EV.Status.apply(G, e, 'slow', 1, pow, true);
  res.reactCooldown = { firstBurst: n1, afterCd: REACTS };
}

// fusion eligibility
EV.DATA.allFusions().forEach((f) => {
  const row = { id: f.id, stage: f.stage, from: f.from, need: f.needGene || f.needPassive };
  try {
    gotoStage(f.stage);
    resetArena();
    row.fromExists = !!EV.DATA.skill(f.from);
    row.needExists = f.needGene ? !!EV.DATA.gene(f.needGene) : !!EV.DATA.passive(f.needPassive);
    const setNeed = (on) => {
      G.legacy.genes = on && f.needGene ? [f.needGene] : [];
      G.build.passives = on && f.needPassive ? [{ id: f.needPassive, rank: 1 }] : [];
      EV.Build.recompute(G);
    };
    const has = () => EV.Build.candidates(G).some((c) => c.type === 'fusion' && c.def.id === f.id);
    G.build.skills = [{ id: f.from, rank: 4, cd: 0 }]; setNeed(true);
    row.rank4 = has();
    G.build.skills = [{ id: f.from, rank: 5, cd: 0 }]; setNeed(false);
    row.rank5_noNeed = has();
    setNeed(true);
    row.rank5_need = has();
    const card = EV.Build.candidates(G).find((c) => c.type === 'fusion' && c.def.id === f.id);
    if (card) {
      EV.Build.apply(G, card);
      const c2 = EV.Build.candidates(G);
      row.afterApply = G.build.skills.map((s) => s.id + ':' + s.rank + (s.fused ? 'F' : ''));
      row.fusionStillOffered = c2.some((c) => c.type === 'fusion');
      row.baseOfferedAgain = c2.some((c) => c.def && c.def.id === f.from);
      row.upSkillForFusion = c2.some((c) => c.type === 'upSkill' && c.def.id === f.id);
      // cast the fused slot through input
      const P = G.player; const tg = spawnTargets(f.stage); P.lockTarget = tg[3];
      P.energy = P.stats.maxEnergy;
      const e0 = P.energy;
      const ground = (f.kind === 'zone' || f.kind === 'leap');
      EV.Input._press('KeyQ'); tick(1); if (ground) tick(2); EV.Input._release('KeyQ'); tick(1);
      row.fusedInputCast = G.build.skills[0].cdMax != null; row.fusedEnergy = +(e0 - P.energy).toFixed(1);
      tick(60); row.fusedDmg = Math.round(hpDmg(tg));
      // rollback (dehset death) restores original
      EV.Build.rollback(G);
      row.afterRollback = G.build.skills.map((s) => s.id + ':' + s.rank);
    }
    // wrong stage
    const other = (f.stage + 1) % 3;
    G.stageIndex = other;
    G.build.skills = [{ id: f.from, rank: 5, cd: 0 }]; setNeed(true);
    row.wrongStage = has();
    G.stageIndex = f.stage;
  } catch (e) { row.exc = shortErr(e); }
  res.fusions.push(row);
});
return res;
