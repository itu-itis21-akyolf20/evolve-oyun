const DIFF='normal', PICK='random'; const PRE=()=>{ const S=EV.CFG.STAGES; S[0].evoMax=8800; S[1].evoMax=20000; S[2].evoMax=44000; EV.CFG.ENDLESS.evoMax=50000; EV.MOBS.APEX.forEach(a=>{a.leash=80; a.hunt=40;}); const M=EV.MOBS.ENEMIES; const sp={whip:7.6,fox:10.0,serpent:8.8,saber:9.6}; M.forEach(p=>p.forEach(d=>{ if(sp[d.id]) d.speed=sp[d.id]; })); };
