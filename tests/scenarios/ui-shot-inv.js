...
T.start('normal'); T.sim(10); G.inv.essence = 99999; EV.Items.SLOTS.forEach((s) => { EV.Items.craft(G, s.id); EV.Items.craft(G, s.id); }); EV.Inv.open(G); G.pause(); const c = document.querySelector('#invBag .icell:not(.empty)'); if (c) c.click(); return audit();
