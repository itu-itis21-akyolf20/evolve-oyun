...
// ---- sk-echo-overlap: echo of the same skill the player currently has active (endless mode: echo pool == build pool) ----
gotoStage(2);
const out = {};
// buff
resetArena(); spawnTargets(2);
const P = G.player;
EV.Skills.cast(G, EV.DATA.skill('m_scent'), 5, { source: 'player' });
tick(1);
out.buffBefore = +P.buffs.find((b) => b.id === 'm_scent').t.toFixed(2);
G.legacy.echoes = [{ id: 'm_scent', rank: 1, t: 0 }];
tick(1);
const b = P.buffs.find((x) => x.id === 'm_scent');
out.buffAfterEcho = b ? +b.t.toFixed(2) : null;
out.buffModsAfterEcho = b ? b.mods : null;
// orbit
resetArena(); const tg = spawnTargets(2);
EV.Skills.cast(G, EV.DATA.skill('m_quills'), 5, { source: 'player' });
tick(1);
out.orbitsBefore = EV.Skills.counts.orbits;
G.legacy.echoes = [{ id: 'm_quills', rank: 1, t: 0 }];
tick(1);
out.orbitsAfterEcho = EV.Skills.counts.orbits;
tick(60);
out.orbitsAt2s = EV.Skills.counts.orbits;
out.note = 'm_quills r5 dur 8 / 6 quills; echo r1 dur 6 / floor(3/2)=1 quill';
return out;
