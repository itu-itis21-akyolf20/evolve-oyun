...
G.onDeath = function () {
  const s = cur();
  if (this.player.alive) { s.deaths++; const by = this.lastHitBy || '?'; s.deathBy[by] = (s.deathBy[by] || 0) + 1;
...
const until = (g) => {
  ...
  return g.generation >= 2 || g.time > MAXMIN * 60;
};
const w0 = Date.now();
