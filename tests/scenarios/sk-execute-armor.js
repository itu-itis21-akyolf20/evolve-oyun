...
// ---- sk-execute-armor: execute reaction vs armored enemy below 22% hp ----
gotoStage(1);
const out = [];
[0, 0.1, 0.3].forEach((armor) => {
  resetArena();
  ...
  EV.Status.apply(G, e, 'vuln', 1, pow, true);
  EV.Status.apply(G, e, 'stun', 1, pow, true);
  out.push({ armor, hpBefore: hp0, hpAfter: e.alive ? e.hp : 0, killed: !e.alive, reacts: REACTS });
});
const shell = EV.MOBS.ENEMIES[1].find((d) => d.id === 'shell');
return { out, shellArmor: shell && shell.armor };
