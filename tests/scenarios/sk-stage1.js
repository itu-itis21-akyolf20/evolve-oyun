...
// ---- sk-stage1: every active + ult of stage 1, rank 1 and 5, direct cast + input path ----
gotoStage(1);
const defs = EV.DATA.stageSkills(1, 'active').concat(EV.DATA.stageSkills(1, 'ult'));
const out = [];
defs.forEach((d) => {
  [1, 5].forEach((rk) => { out.push(runSkill(d, rk, 'direct')); out.push(runSkill(d, rk, 'input')); });
});
return out;
