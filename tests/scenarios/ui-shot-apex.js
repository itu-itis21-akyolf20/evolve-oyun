...
T.start('normal'); T.sim(5); const a = EV.Enemies.spawnApex(G); const P = G.player; const f = new THREE.Vector3(Math.sin(P.yaw), 0, Math.cos(P.yaw));
 const x = P.group.position.x + f.x * 11, z = P.group.position.z + f.z * 11; a.group.position.set(x, W.groundY(x, z), z); step(2); G.paused = true; return Object.assign(audit(), { apexDist: +a.group.position.distanceTo(P.group.position).toFixed(1), fleeShown: !document.getElementById('fleeWarn').hidden, vig: document.getElementById('dangerVig').style.opacity });
